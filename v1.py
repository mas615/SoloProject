...
hidcanary = result[0]['value'] # 히든 까나리 저장 ------------------------------> 로그인에 실패하였습니다 (여기에서 오류가 자주 발생하니 try catch 하는게 좋을듯)
...
for i in soup2result :
    ...
    for j in chkmsg:
        ...
        res4html = session.get(url4, cookies=cookies2, verify = False)
        soup4 = BeautifulSoup(res4.text,"html.parser")
        #html = str(soup4.find('td',{'class':'cntnttp'}))
        sub = soup4.find('td',{'class':'sub'}).get_text() #에러처리 필요
        sub3 = ''.join(char for char in sub if char.isalnum() or char in (' ', '_', '-'))
        time = soup4.find('td',{'class':'hdtxnr'}).get_text() #에러처리 필요
        ...
        #failedfolder += foldername+'-'+time2+sub3+'\n'
...
#print(failedfolder)
#5. 폴더 들어가서 체크박스 가져오기
# folderurl = 'https://mail.partner.skcc.com/owa/?ae=Folder&t='+soup2result[2]['value']+'&slUsng=1'
# folderres = requests.get(folderurl, cookies=cookies2, verify = False)
# soup3 = BeautifulSoup(folderres.content,"html.parser")
# chkmsg = soup3.find_all('input',{'name':'chkmsg'})

# for i in chkmsg:
    # url4 = 'https://mail.partner.skcc.com/owa/?ae=Item&t=IPM.Note&id='+i['value']
    # res4 = requests.get(url4, cookies=cookies2, verify = False)
    # soup4 = BeautifulSoup(res4.content,"html.parser")
    # html = str(soup4.find_all('td',{'class':'cntnttp'})[0])
    # file = open(i['value']+".html","w", encoding='utf-8')
    # file.write(html)
    # file.close()
