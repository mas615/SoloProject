#© 2023 MaJunYoung akwns615@gmail.com v14
...
from tkinter import * 
copy = '© 2023 MaJunYoung akwns615@gmail.com v14'
#RequestsCookieJar는 dict 처럼 작동
#2000414@partner.skcc.com
#2300091@partner.skcc.com
print(copy)
mailid = pyautogui.prompt(title='ID', text='아이디를 입력하세요', default='2300091@partner.skcc.com')
...
while whilelogin < 3000:
    ...
    # print(len(str(res100.text)))
    if len(str(res.text)) > 3000 and len(str(res100.text)) > 3000:
...
#버튼
btn = Button(root, text="추가", width=10, command=lambda: btnpress())
btn.pack(side=LEFT)
btn2 = Button(root, text="완료", width=10, command=lambda: root.destroy())
...
for i in soup2result :    
    ...
    if foldername in vari2:  #--------------------------------------------------------------vari2로 선택한 폴더만 저장하게 코딩
        ...
        for j in chkmsg:
            ...
            # session.proxies = {'http': 'http://127.0.0.1:8088', 'https': 'http://127.0.0.1:8088'}
            #뉴 세션
            # urlorigin = "https://mail.partner.skcc.com/owa/auth.owa"
            # datasorigin = {'destination':'https://mail.partner.skcc.com/owa/','flags':'0','forcedownlevel':'0','trusted':'0','username':mailid,'password':mailpw,'isUtf8':'1'}
            # cookiesorigin = {'OutlookSession':'b1a7d7bd6ef645549c4a633c2f0c3adb','PBack':'0'}
            # headersorigin = {'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.111 Safari/537.36'}
            #res = session2.post(urlorigin, data=datasorigin, cookies=cookiesorigin, headers=headersorigin, verify = False)
